...
import { ConnectWallet, useAddress, useSDK, useContractRead } from "@thirdweb-dev/react";
  ...
  useEffect(() => {
    ...
    .then(async (contracts) => {
      const formattedContracts = await Promise.all(contracts.map(async (contract) => ({
        ...
        contractType: await contract.contractType(),
      })));
      setExistingContracts(formattedContracts);
    })
